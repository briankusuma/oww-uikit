...
4. **Struktur Navigasi Atas (Top Navbar Links)**: Navigasi atas (`.doc-navbar__nav`) hanya terdiri dari 2 menu: **Docs** (selalu aktif dengan `is-active`, mengarah ke `introduction.html`) dan **Example** (hanya teks tanpa link aktif: `href="#" onclick="return false;"`). Brand logo di header juga mengarah ke `introduction.html`. Jangan mencantumkan *Components* atau *Tokens* di navbar atas.
5. **Sidebar Getting Started**: Item pertama pada grup "Getting Started" adalah **Introduction** (`introduction.html`), diikuti dengan Overview dan Architecture & Rules. Halaman `introduction.html` adalah panduan pengenalan awal design system, prinsip desain, instalasi, dan ringkasan komponen.
